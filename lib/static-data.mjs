...
function escapeReplace(value) {
  return String(value).replace(/\$/g, '$$$$');
}

export function injectPublicFaqPrices(html, payload) {
  try {
    const sentence = escapeReplace(escapePublicHtml(renderMenuSentence(payload)));
    const jsonSentence = escapeReplace(renderMenuSentence(payload).replace(/\\/g, '\\\\').replace(/"/g, '\\"'));
    ...
    if (kids) {
      const price = escapeReplace(escapePublicHtml(kids.precio_formateado));
      const plain = escapeReplace(kids.precio_formateado);
      next = next.replace(
